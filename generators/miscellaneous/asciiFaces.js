...
function generator$miscellaneous$asciiFaces(type) {
    var names = [];
    for(var i = 0; i < 10; i++) {
        var a = Math.floor(Math.random() * faces.length);
        var b = Math.floor(Math.random() * eyes.length);
        var mid = mids[Math.floor(Math.random() * mids.length)];
        names.push(faces[a][0] + eyes[b][0] + mid + eyes[b][1] + faces[a][1]);
    }
    return names;
}
